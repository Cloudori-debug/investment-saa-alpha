...
from alpha_system.ui.services.weekly_qual_report import DOMAIN_KEYS, load_weekly_suggestions

SUGGESTIONS_NAME = "weekly_qual_suggestions.json"
...
def mark_sources_reviewed(
    *,
    ...
    payload = _load(root)
    if not payload:
        raise ValueError("주간 정성 제안 파일이 없습니다. 먼저 업로드하세요.")
...
def approve_domain(
    *,
    ...
    payload = _load(root)
    if not payload:
        raise ValueError("주간 정성 제안 파일이 없습니다.")
...
def _load(root: Path) -> dict[str, Any]:
    return load_weekly_suggestions(root)


def _save(root: Path, payload: dict[str, Any]) -> None:
    path = root / "data" / SUGGESTIONS_NAME
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _journal_rescore_hook(
    *,
    ...
    try:
        ...
        from alpha_system.scoring.rescore import evaluate_rescore_triggers
