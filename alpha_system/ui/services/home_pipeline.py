...
def build_pipeline_stages(ctx: DashboardContext) -> list[PipelineStage]:
    ...
    if cecs_total < 30 or cecs_final < 30:
        stages.append(
            PipelineStage(
                ...
                title="CECS 승인",
                status="locked",
                reason=f"CECS final {cecs_final}/{cecs_total} — 주간 정성에서 30종 final 승인이 필요합니다.",
                cta_label="결재함 CECS",
                page=PAGE_APPROVAL,
                focus=FOCUS_WEEKLY_QUAL,
                blocks_next=True,
        ...
        stages.append(
            PipelineStage(
                ...
                title="CECS 승인",
                status="ok",
                reason=f"CECS final {cecs_final}/{cecs_total}",
                cta_label="결재함 보기",
                page=PAGE_APPROVAL,
                focus=FOCUS_WEEKLY_QUAL,
                blocks_next=False,
    ...
    # 6) Ops book
    ops_n = int(getattr(ctx, "held_kr_alpha", 0) or 0)
    if ops_n <= 0:
        stages.append(
            PipelineStage(
                key="ops",
                ...
                title="운용 북",
                status="locked",
                reason="실보유(ops_book)가 비어 있습니다. 제안 승인·수동 체결 후 반영됩니다.",
                cta_label="포트폴리오",
                page=PAGE_PORTFOLIO,
                blocks_next=False,
            )
        ...
        stages.append(
            PipelineStage(
                ...
                title="운용 북",
                status="ok",
                reason=f"ops_book {ops_n}종 보유 · target 자동변경 없음",
                cta_label="운용 북 보기",
                page=PAGE_PORTFOLIO,
                blocks_next=False,
            )
