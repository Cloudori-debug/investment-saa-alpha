"""Portfolio detail — proposal_book / ops_book tabs."""
...
def render_portfolio(ctx: DashboardContext) -> None:
    ...
    tab_proposal, tab_ops = st.tabs(["제안 북 (스크린)", "운용 북 (실보유)"])
    with tab_proposal:
        st.caption(
            "proposal_book · 매입 비중 없음 · eligibility 통과 후 allocate_tranche 제안 비중"
        )
    ...
    with tab_ops:
        st.caption(
            "ops_book · positions.csv 실보유 비중 · cap/감축·exit 판정의 기준"
        )
        ops_rows = getattr(ctx, "ops_portfolio_rows", None) or []
        if not ops_rows:
            st.info("kr_alpha 실보유가 없습니다.")
        else:
            _render_ops_book(ctx, ops_rows)
...
def _render_ops_book(ctx: DashboardContext, rows: list[PortfolioRow]) -> None:
    """Temporarily swap portfolio_rows so existing widgets render ops_book."""
    original = ctx.portfolio_rows
    try:
        ctx.portfolio_rows = rows
        render_portfolio_bullets(ctx, mode="full", key_prefix="pf_ops")
    finally:
        ctx.portfolio_rows = original
