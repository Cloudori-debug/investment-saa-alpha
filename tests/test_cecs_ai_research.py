"""CECS AI research report contract and audit journal."""
...
def test_request_is_one_fixed_schema_batch_file(tmp_path: Path) -> None:
    clear_entries()
    generated = datetime(2026, 7, 17, 8, 30, 0)
    report = write_cecs_ai_research_report(
        subjects=[
            CecsResearchSubject(
                ticker="005830",
                name="DB손해보험",
    ...
    assert report.path.name == "cecs_ai_research_batch_20260717.md"
    assert report.path.exists()
    assert USAGE_WARNING in report.markdown
    assert "## [005830] DB손해보험" in report.markdown
    assert "### execution" in report.markdown
    assert "### pension" in report.markdown
    assert "### purpose" in report.markdown
    assert report.markdown.count("- 점수 제안(0-100): _____") == 3
    assert report.markdown.count("- 근거: _____") == 3
    assert report.markdown.count("- 출처:") == 3
    assert "확인 불가" in report.markdown
    assert "추정하지 않는다" in report.markdown
    assert report.provider_status == "EXTERNAL_FILL_REQUIRED"

    entries = list_entries(action_kind="CECS_AI_RESEARCH_GENERATED")
    assert len(entries) == 1
    assert entries[0].subject == "batch"
    assert entries[0].payload["tickers"] == ["005830"]
    assert categorize("CECS_AI_RESEARCH_GENERATED") == "데이터"
    clear_entries()


def test_batch_report_uses_one_file_with_per_ticker_sections(tmp_path: Path) -> None:
    clear_entries()
    subjects = [
        CecsResearchSubject("005830", "DB손해보험", "insurance"),
        CecsResearchSubject("021240", "코웨이", "consumer"),
    ]
    report = write_cecs_ai_research_report(
        subjects=subjects,
        docs_dir=tmp_path / "docs",
    ...
    assert "## [005830] DB손해보험" in report.markdown
    assert "## [021240] 코웨이" in report.markdown
    assert report.markdown.count("- 점수 제안(0-100): _____") == 6
    assert report.tickers == ("005830", "021240")

    entry = list_entries(action_kind="CECS_AI_RESEARCH_GENERATED")[0]
    assert entry.subject == "batch"
    assert entry.payload["mode"] == "batch"
    assert entry.payload["tickers"] == ["005830", "021240"]
    clear_entries()
