...
def test_cutoff_blocked_before_30_and_requires_two_confirmations(
    tmp_path: Path,
) -> None:
    root = Path(__file__).resolve().parents[1]
    config = tmp_path / "alpha_system.yaml"
    shutil.copy2(root / "alpha_system" / "config" / "alpha_system.yaml", config)
    cecs = _template(tmp_path / "cecs.csv", final_n=29)
    original = config.read_bytes()

    with pytest.raises(ValueError, match="30/30|완료 전"):
        confirm_score_cutoff(
            config_path=config,
            cecs_path=cecs,
            cutoff=55.0,
